...
"""
    ...
    def show_current_sample(self):
        ...
        # --- Enhanced label display for n-grams (unigram, bigram, trigram) ---
        ascii_names = {
        ...
        def ascii_name(c):
            if c in ascii_names:
                ...
                return f"0x{c:02X}"
        # Trigram label: 24 bits
        if label > 0xFFFF:
            char1 = (label >> 16) & 0xFF
            char2 = (label >> 8) & 0xFF
            char3 = label & 0xFF
            chars = [ascii_disp(char1), ascii_disp(char2), ascii_disp(char3)]
            label_str = f"Label: {label} (Hex: {label:06X}, Chars: {''.join(chars)})"
            self.label_text.config(state=tk.NORMAL)
            self.label_text.delete(1.0, tk.END)
            self.label_text.insert(tk.END, label_str)
            self.label_text.config(state=tk.DISABLED)
            self.char_name_var.set(f"{ascii_name(char1)} {ascii_name(char2)} {ascii_name(char3)}")
        # Bigram label: 16 bits
        elif label > 255:
            char1 = label >> 8
            char2 = label & 0xFF
            chars = [ascii_disp(char1), ascii_disp(char2)]
            label_str = f"Label: {label} (Hex: {label:04X}, Chars: {''.join(chars)})"
            self.label_text.config(state=tk.NORMAL)
            self.label_text.delete(1.0, tk.END)
            self.label_text.insert(tk.END, label_str)
            self.label_text.config(state=tk.DISABLED)
            self.char_name_var.set(f"{ascii_name(char1)} {ascii_name(char2)}")
        # Unigram label: 8 bits
        else:
            char_display = ascii_disp(label)
            label_str = f"Label: {label} (Hex: {label:02X}, Char: {char_display})"
            self.label_text.config(state=tk.NORMAL)
            self.label_text.delete(1.0, tk.END)
            self.label_text.insert(tk.END, label_str)
            self.label_text.config(state=tk.DISABLED)
        ...
        # Convert features to ASCII text (with trigram support comments)
        ...
        try:
            ...
            if is_binary:
                ...
                bytes_data = []
                # For trigrams, features may be grouped in 3-byte chunks (24 bits),
                # but we still process as bytes for display.
